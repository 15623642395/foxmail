...
public class MainListMouseListener extends MouseAdapter {
	...
	public void handle(FileObject file) {
		int result = JOptionPane.showOptionDialog(null, "请选择操作", "选择", 0, JOptionPane.QUESTION_MESSAGE, null,
	...
	public void openFile(FileObject file) {
		try {
			...
			e.printStackTrace();
			JOptionPane.showConfirmDialog(null, "打开文件错误, " + file.getSourceName() + "文件不存在", "错误",
...
class FolderChooser extends JFileChooser {
	...
	public void approveSelection() {
		...
		if (targetFile.isDirectory()) {
			...
			File newFile = new File(targetFile.getAbsolutePath() + File.separator + this.sourceFile.getSourceName());
