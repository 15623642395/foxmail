...
public class PropertiesUtil {
	...
	public static MailContext createContext(File propertyFile) throws IOException {
		...
		return new MailContext(null, props.getProperty("account"), props.getProperty("password"),
				props.getProperty("smtpHost"), smtpPort, props.getProperty("pop3Host"), pop3Port);
	}
	...
	public static void store(MailContext ctx) {
		try {
			File propFile = new File(FileUtil.DATE_FOLDER + ctx.getUser() + FileUtil.CONFIG_FILE);
			...
			throw new PropertiesException("请检查系统的配置文件, " + FileUtil.DATE_FOLDER + ctx.getUser() + FileUtil.CONFIG_FILE);
		}
	}
}
